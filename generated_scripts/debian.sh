...
_debian_install_1() {
	...
	if [ -f /etc/apt/sources.list.d/ubuntu.sources ]; then
		...
		[ -f ${_backup_dir}/debian_first__etc_apt_sources.list.d_ubuntu.sources.bak ] || $sudo cp /etc/apt/sources.list.d/ubuntu.sources ${_backup_dir}/debian_first__etc_apt_sources.list.d_ubuntu.sources.bak || {
	...
	if [ -f /etc/apt/sources.list ]; then
		...
		[ -f ${_backup_dir}/debian_first__etc_apt_sources.list.bak ] || $sudo cp /etc/apt/sources.list ${_backup_dir}/debian_first__etc_apt_sources.list.bak || {
...
uninstall() {
	...
	if [ -f ${_backup_dir}/debian_first__etc_apt_sources.list.d_ubuntu.sources.bak ]; then
		set_sudo
		$sudo cp "${_backup_dir}/debian_first__etc_apt_sources.list.d_ubuntu.sources.bak" /etc/apt/sources.list.d/ubuntu.sources 2>/dev/null || true
		print_info "Restored /etc/apt/sources.list.d/ubuntu.sources"
	fi
	if [ -f ${_backup_dir}/debian_first__etc_apt_sources.list.bak ]; then
		set_sudo
		$sudo cp "${_backup_dir}/debian_first__etc_apt_sources.list.bak" /etc/apt/sources.list 2>/dev/null || true

...
_deepin_install_1() {
	...
	if [ -f "/etc/apt/sources.list" ]; then
		...
		$sudo cp "/etc/apt/sources.list" "${_backup_dir}/deepin_1_1.bak" || {
			print_error "Backup /etc/apt/sources.list failed"
			return 1
		}
		$sudo sed -i -E 's|https?://([^/]+)/deepin|$http://$domain/deepin|g' "/etc/apt/sources.list" || {
	...
	if [ -f "/etc/apt/sources.list" ]; then
		...
		$sudo cp "/etc/apt/sources.list" "${_backup_dir}/deepin_1_2.bak" || {
			print_error "Backup /etc/apt/sources.list failed"
			return 1
		}
		$sudo sed -i -E 's|https?://([^/]+)/beige|$http://$domain/deepin/beige|g' "/etc/apt/sources.list" || {
			print_error "Failed to update /etc/apt/sources.list"
			return 1
		}
	else
...
uninstall() {
	...
	if [ -f "${_backup_dir}/deepin_1_1.bak" ]; then
		set_sudo
		$sudo cp "${_backup_dir}/deepin_1_1.bak" "/etc/apt/sources.list" 2>/dev/null || true
		print_info "Restored /etc/apt/sources.list"
	fi
	if [ -f "${_backup_dir}/deepin_1_2.bak" ]; then
		set_sudo
		$sudo cp "${_backup_dir}/deepin_1_2.bak" "/etc/apt/sources.list" 2>/dev/null || true
		print_info "Restored /etc/apt/sources.list"
	fi

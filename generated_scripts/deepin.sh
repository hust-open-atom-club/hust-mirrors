...
_deepin_install_1() {
	...
	if [ -f /etc/apt/sources.list ]; then
		...
		[ -f ${_backup_dir}/deepin_first__etc_apt_sources.list.bak ] || $sudo cp /etc/apt/sources.list ${_backup_dir}/deepin_first__etc_apt_sources.list.bak || {
...
uninstall() {
	...
	if [ -f ${_backup_dir}/deepin_first__etc_apt_sources.list.bak ]; then
		set_sudo
		$sudo cp "${_backup_dir}/deepin_first__etc_apt_sources.list.bak" /etc/apt/sources.list 2>/dev/null || true
...
can_recover() {
	...
	[ -f ${_backup_dir}/deepin_1_1.bak ] || [ -f ${_backup_dir}/deepin_1_2.bak ]
}

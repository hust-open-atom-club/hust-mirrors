...
check() {
	# No detection rules specified for openeuler
	return 0
}
...
_openeuler_install_1() {
	...
	if [ -f "/etc/yum.repos.d/openEuler.repo" ]; then
		...
		$sudo cp "/etc/yum.repos.d/openEuler.repo" "${_backup_dir}/openeuler_1_1.bak" || {
			print_error "Backup /etc/yum.repos.d/openEuler.repo failed"
			return 1
		}
		$sudo sed -i -E 's|http://repo.openeuler.org|$http://$domain/openeuler|g' "/etc/yum.repos.d/openEuler.repo" || {
	...
	if [ -f "/etc/yum.repos.d/openEuler.repo" ]; then
		...
		$sudo cp "/etc/yum.repos.d/openEuler.repo" "${_backup_dir}/openeuler_1_2.bak" || {
			print_error "Backup /etc/yum.repos.d/openEuler.repo failed"
			return 1
		}
		$sudo sed -i -E 's|\\(metalink=.*$\\)|# \\1|g' "/etc/yum.repos.d/openEuler.repo" || {
			print_error "Failed to update /etc/yum.repos.d/openEuler.repo"
			return 1
		}
	else
...
uninstall() {
	...
	if [ -f "${_backup_dir}/openeuler_1_1.bak" ]; then
		set_sudo
		$sudo cp "${_backup_dir}/openeuler_1_1.bak" "/etc/yum.repos.d/openEuler.repo" 2>/dev/null || true
		print_info "Restored /etc/yum.repos.d/openEuler.repo"
	fi
	if [ -f "${_backup_dir}/openeuler_1_2.bak" ]; then
		set_sudo
		$sudo cp "${_backup_dir}/openeuler_1_2.bak" "/etc/yum.repos.d/openEuler.repo" 2>/dev/null || true
		print_info "Restored /etc/yum.repos.d/openEuler.repo"
	fi
...
is_deployed() {
	...
	[ -f "/etc/yum.repos.d/openEuler.repo" ] && grep -q "$domain" "/etc/yum.repos.d/openEuler.repo" 2>/dev/null && return 1
	return 0
}

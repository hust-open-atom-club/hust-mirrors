...
_alpine-linux_install_1() {
	...
	if [ -f "/etc/apk/repositories" ]; then
		...
		$sudo cp "/etc/apk/repositories" "${_backup_dir}/alpine-linux_1_1.bak" || {
			print_error "Backup /etc/apk/repositories failed"
			return 1
		}
		$sudo sed -i -E 's|dl-cdn\.alpinelinux\.org|$domain|g' "/etc/apk/repositories" || {
...
uninstall() {
	...
	if [ -f "${_backup_dir}/alpine-linux_1_1.bak" ]; then
		set_sudo
		$sudo cp "${_backup_dir}/alpine-linux_1_1.bak" "/etc/apk/repositories" 2>/dev/null || true
...
is_deployed() {
	...
	[ -f "/etc/apk/repositories" ] && grep -q "$domain" "/etc/apk/repositories" 2>/dev/null && return 1
	return 0
}

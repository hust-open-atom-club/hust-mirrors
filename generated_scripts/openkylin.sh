...
check() {
	# No detection rules specified for openkylin
	return 0
}
...
_openkylin_install_1() {
	...
	if [ -f "/etc/apt/sources.list" ]; then
		...
		$sudo cp "/etc/apt/sources.list" "${_backup_dir}/openkylin_1_1.bak" || {
			print_error "Backup /etc/apt/sources.list failed"
			return 1
		}
		$sudo sed -i -E 's|https?://([^/]+)/openkylin|$http://$domain/openkylin|g' "/etc/apt/sources.list" || {
...
uninstall() {
	...
	if [ -f "${_backup_dir}/openkylin_1_1.bak" ]; then
		set_sudo
		$sudo cp "${_backup_dir}/openkylin_1_1.bak" "/etc/apt/sources.list" 2>/dev/null || true

...
_arch-linux-cn_install_1() {
	...
	if [ -f /etc/pacman.conf ]; then
		...
		[ -f ${_backup_dir}/arch-linux-cn_first__etc_pacman.conf.bak ] || $sudo cp /etc/pacman.conf ${_backup_dir}/arch-linux-cn_first__etc_pacman.conf.bak || {
...
uninstall() {
	...
	if [ -f ${_backup_dir}/arch-linux-cn_first__etc_pacman.conf.bak ]; then
		set_sudo
		$sudo cp "${_backup_dir}/arch-linux-cn_first__etc_pacman.conf.bak" /etc/pacman.conf 2>/dev/null || true

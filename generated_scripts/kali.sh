...
_kali_install_1() {
	...
	if [ -f /etc/apt/sources.list ]; then
		...
		[ -f ${_backup_dir}/kali_first__etc_apt_sources.list.bak ] || $sudo cp /etc/apt/sources.list ${_backup_dir}/kali_first__etc_apt_sources.list.bak || {
...
uninstall() {
	...
	if [ -f ${_backup_dir}/kali_first__etc_apt_sources.list.bak ]; then
		set_sudo
		$sudo cp "${_backup_dir}/kali_first__etc_apt_sources.list.bak" /etc/apt/sources.list 2>/dev/null || true

...
check() {
	# No detection rules specified for linux-mint
	return 0
}
...
_linux-mint_install_1() {
	...
	if [ -f "/etc/apt/offical-package-repositories.list" ]; then
		...
		$sudo cp "/etc/apt/offical-package-repositories.list" "${_backup_dir}/linux-mint_1_1.bak" || {
			print_error "Backup /etc/apt/offical-package-repositories.list failed"
			return 1
		}
		$sudo sed -i -E 's|http://packages.linuxmint.com|$http://$domain/linuxmint|g' "/etc/apt/offical-package-repositories.list" || {
	...
	if [ -f "/etc/apt/offical-package-repositories.list" ]; then
		...
		$sudo cp "/etc/apt/offical-package-repositories.list" "${_backup_dir}/linux-mint_1_2.bak" || {
			print_error "Backup /etc/apt/offical-package-repositories.list failed"
			return 1
		}
		$sudo sed -i -E 's|http://archive.ubuntu.com|$http://$domain|g' "/etc/apt/offical-package-repositories.list" || {
			print_error "Failed to update /etc/apt/offical-package-repositories.list"
			return 1
		}
	else
...
uninstall() {
	...
	if [ -f "${_backup_dir}/linux-mint_1_1.bak" ]; then
		set_sudo
		$sudo cp "${_backup_dir}/linux-mint_1_1.bak" "/etc/apt/offical-package-repositories.list" 2>/dev/null || true
		print_info "Restored /etc/apt/offical-package-repositories.list"
	fi
	if [ -f "${_backup_dir}/linux-mint_1_2.bak" ]; then
		set_sudo
		$sudo cp "${_backup_dir}/linux-mint_1_2.bak" "/etc/apt/offical-package-repositories.list" 2>/dev/null || true
		print_info "Restored /etc/apt/offical-package-repositories.list"
	fi
...
is_deployed() {
	...
	[ -f "/etc/apt/offical-package-repositories.list" ] && grep -q "$domain" "/etc/apt/offical-package-repositories.list" 2>/dev/null && return 1
	return 0
}

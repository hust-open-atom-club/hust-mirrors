...
Golang 富有表现力、简洁、干净且高效。它的并发机制使编写能够充分利用多核和联网机器的程序变得容易，
...
1. 首先在[下载页面](/release/?release=Golang)下载对应系统和架构的安装包，如您是Linux系统，您可使用如下命令下载：

    ```shell varcode
    # use wget
    wget ${_http}://${_domain}/golang/go<version>/go<version>.linux-<arch>.tar.gz
    # use curl
    curl -O ${_http}://${_domain}/golang/go<version>/go<version>.linux-<arch>.tar.gz
    ```
